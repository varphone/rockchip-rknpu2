...
int NC1HWC2_to_NCHW(const int8_t *src, float *dst, uint32_t *dims, int channel, int hw_dst, int zp, float scale)
{
  int batch = dims[0];
  int C1 = dims[1];
  int h = dims[2];
...
int main(int argc, char *argv[])
{
  ...
  for (uint32_t i = 0; i < io_num.n_output; i++)
  {
    ...
    NC1HWC2_to_NCHW((int8_t *)output_mems[i]->virt_addr, (float *)output_mems_nchw[i]->virt_addr, output_attrs[i].dims,
                    channel, hw, zp, scale);
  }
  ...
  for (uint32_t i = 0; i < io_num.n_output; i++)
  {
    ...
    float *buffer = (float *)output_mems_nchw[i]->virt_addr;
